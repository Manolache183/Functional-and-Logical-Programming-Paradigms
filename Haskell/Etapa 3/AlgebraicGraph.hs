...
-}
recursiveOverlay :: [a] -> AlgebraicGraph a
recursiveOverlay [] = Empty
recursiveOverlay (x:xs) = Overlay (Node x) (recursiveOverlay xs) -- efectiv le pun gramada
...
splitNode node targets = extend (\n -> if n == node then (recursiveOverlay targets) else Node n)
