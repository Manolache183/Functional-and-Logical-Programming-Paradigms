...
helperSearch f stq graph visited
    | (null stq) = visited -- stack / queue gol
    | (elem (head stq) visited) = helperSearch f (tail stq) graph visited -- am vizitat deja elementul
    | otherwise = helperSearch f (f stq (S.toList (outNeighbors (head stq) graph))) graph (visited ++ [head stq]) -- vizitam elementul
...
search f node graph = helperSearch f [node] graph []
...
countIntermediate from to graph =
    let bfsList = bfs from graph
        dfsList = dfs from graph
    in
    if (not (elem to (bfs from graph)) || not (elem to (dfs from graph))) then Nothing
    else Just ((length (fst (span (\ x -> x /= to) bfsList)) - 1), (length (fst (span (\ x -> x /= to) dfsList)) - 1)) -- numeri efectiv
