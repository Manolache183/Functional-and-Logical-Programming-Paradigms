...
; queue reprezinta din care elementul curent este varful
; adaugam in coada toti "vecinii" elementului curent

(define ppt-stream-in-tree-order
  (let search ([queue (stream-cons '(3 4 5) empty-stream)])
    (let ([curr (stream-first queue)])
      (stream-cons curr (search (stream-append (stream-rest queue) (next-transf curr)))))))

(define (next-transf V)
  (stream (multiply T1 V) (multiply T2 V) (multiply T3 V)))
...
; prima data printam o pereche, apoi 2, samd, mereu punem o limita de cat de mult incrementam g ul
; daca se respecta limita, printam primul element din G si primul din H, incrementam G ul si size ul curent
; daca s a ajuns la limita, resetam G ul si size ul curent, incrementam H si limita

(define (pairs G H)
  (pairs-helper2 G H G 1 1))

(define (pairs-helper2 G H original_G curr limit)
  (if (<= curr limit)
      (stream-cons (cons (stream-first G) (stream-first H)) (pairs-helper2 (stream-rest G) H original_G (+ 1 curr) limit))
      (pairs-helper2 original_G (stream-rest H) original_G 1 (+ 1 limit))))
...
; fluxurile nr impare, dintre cu start 1 sau 3
; filter sa aiba gcd 1 si sa fie relatia de < intre ele

(define (my-flux-from n) 
  (stream-cons n (my-flux-from (+ 2 n))))

(define G-flux (my-flux-from 1))

(define H-flux (my-flux-from 3))

(define gh-pairs-stream
  (stream-filter
   (λ (pair)
     (let ([g (car pair)] [h (cdr pair)])
       (if (and (< g h) (= 1 (gcd g h)))
           #t
           #f))) (pairs G-flux H-flux)))
...
(define ppt-stream-in-pair-order
  (let streamuleanu ([gh-stream gh-pairs-stream])
    (let ([g (car (stream-first gh-stream))] [h (cdr (stream-first gh-stream))])
      (stream-cons (list (* g h) (/ (- (* h h) (* g g)) 2) (/ (+ (* h h) (* g g)) 2)) (streamuleanu (stream-rest gh-stream)))))) 
